"""Dependency-free, bilingual profile rendering. No external image services."""
...
def validate_projects(projects: list[dict]) -> None:
    ...
    for project in projects:
        ...
        for key in ("repo", "name", "en", "zh", "web", "icon"):
        ...
        seen.add(repo.lower())
        if project["icon"] not in ICONS or not project["web"].startswith("https://"):
            raise ValueError(f"Invalid icon or website for {repo}")
        # Keep one line per language readable at mobile widths, without clipping.
...
def render_readme(intro: str, ranked: list[dict], counts: dict[str, int], date: str, footer: str = "") -> str:
    featured, others = ranked[:FEATURED_COUNT], ranked[FEATURED_COUNT:]
    lines = ["<!-- Generated by scripts/update_stars.py. Edit profile/*.md and profile/projects.json. -->", "", intro.strip(), "", '<p align="center"><sub>PROJECTS / 项目</sub></p>', "", '<p align="center">']
    for project in featured:
        ...
        lines.append(f'<a href="https://github.com/{project["repo"]}"><picture><source media="(prefers-color-scheme: dark)" srcset="{ASSET_DIR}/{slug}-dark.svg?v={dark_version}"><img src="{ASSET_DIR}/{slug}-light.svg?v={light_version}" width="380" alt="{alt}"></picture></a>')
    lines += ["</p>", "", '<p align="center"><sub>Websites / 官网 · ' + " · ".join(f'<a href="{escape(p["web"], quote=True)}">{escape(p["name"])}</a>' for p in featured) + "</sub></p>", ""]
    if others:
        lines += ["<details>", f"<summary>More projects / 其他项目 · {len(others)}</summary>", "<br>", ""]
        for project in others:
            lines += [f'<p><a href="https://github.com/{project["repo"]}"><strong>{escape(project["name"])}</strong></a> &nbsp; <sub>☆ {counts[project["repo"]]:,} · <a href="{escape(project["web"], quote=True)}">Website / 官网 ↗</a></sub><br>', f'<sub>{escape(project["en"])}<br>{escape(project["zh"])}</sub></p>', ""]
        lines += ["</details>", ""]
    if footer.strip():
        lines += [footer.strip(), ""]
